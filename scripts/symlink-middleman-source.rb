...
Dir.chdir('source') do |dir|
  ...
  Dir.chdir('datasources') do |ds_dir|
    Dir.glob('../../content/datasources/*.md') do |src|
      md = "_#{File.basename(src)}.erb"
      next if %w(_ruby.md.erb _rails.md.erb).include?(md)
      make_symlink(src, md, ds_dir)
    end
  end
